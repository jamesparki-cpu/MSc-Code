...
STYLE: block colours, display names, decimals and DPI come from
report_style.py. GROUP_DEFS / GROUP_ORDER stay here -- they are the ANALYTICAL
definition of a block and must not move into a presentation module.
"""
...
def grouped_importance(sv: np.ndarray, feats: Sequence[str]) -> pd.DataFrame:
    ...
    # sorted by magnitude, not GROUP_ORDER: the ranking IS the result. (A
    # GROUP_ORDER lookup used to be built here and never used -- removed.)
...
def plot_bar_individual(imp: pd.DataFrame, title, path, top=20):
    ...
    plt.barh(d.feature, d.mean_abs_shap,
             color=[S.BLOCK_COLOURS.get(g, "#999") for g in d.group],
             edgecolor=S.BAR_EDGE, linewidth=0.4)
    plt.xlabel(S.metric_label("mean_abs_shap") +
               " (contribution to predicted suitability)")
    plt.title(title, fontsize=10); plt.grid(axis="x", alpha=0.3)
    # legend so the block colours are readable without cross-referencing
    blocks = S.blocks_in(d.group.unique())
    plt.legend(handles=[Patch(facecolor=S.BLOCK_COLOURS.get(b, "#999"),
                              edgecolor=S.BAR_EDGE, label=S.block_label(b))
...
def plot_bar_grouped(gimp: pd.DataFrame, title, path):
    ...
    plt.barh([S.block_label(g) for g in d.group], d.mean_abs_group_shap,
             color=[S.BLOCK_COLOURS.get(g, "#999") for g in d.group],
    ...
    for y, (v, p, n) in enumerate(zip(d.mean_abs_group_shap, d.pct_of_total, d.n_features)):
        plt.text(v, y, f"  {S.fmt(p, 'pct_of_total')}%  (n={S.fmt(n, 'n_features')})",

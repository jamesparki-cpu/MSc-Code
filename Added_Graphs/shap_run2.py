...
RUN_MODELS = list(S.MODEL_ORDER)      # canonical order, from report_style
...
def run():
    ...
    for variant in RUN_MODELS:
        ...
        log(f"[shap] {variant} top blocks: " +
            ", ".join(f"{S.block_label(r.group)} {S.fmt(r.pct_of_total, 'pct_of_total')}%"
                      for r in gimp.head(3).itertuples()))

    if grouped_all:
        allg = pd.concat(grouped_all, ignore_index=True)
        allg["model"] = pd.Categorical(allg["model"], S.models_in(allg["model"]), ordered=True)
        allg = allg.sort_values(["model"])
        ...
        log(allg.pivot(index="group", columns="model", values="pct_of_total")
                .reindex(S.blocks_in(allg["group"])).round(1).to_string())

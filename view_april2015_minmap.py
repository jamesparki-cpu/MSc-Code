...
img = ax.imshow(tmin_data[0], cmap='coolwarm', vmin=10, vmax=35)

# Add a color scale bar to the side
fig.colorbar(img, ax=ax, label='Min Temp (C)')
ax.set_title("Florida 5km Grid: Min Temp - Day 1")
...
def update(val):
    ...
    img.set_data(tmin_data[day - 1])
    # Update the title
    ax.set_title(f"Florida 5km Grid: Min Temp - Day {day}")
